...
from model_utils import load_checkpoint, load_nemo_checkpoint, load_model, load_sc_model, write_to_log, decode_lm, get_ex_model, load_ex_sc_model
...
@torch.no_grad()
def evaluate(args, model, corpus, decoder, exModel = None):
    ...
    for batch_num, batch in enumerate(pbar):
        ...
        if use_ex_model:        
            ex_model_out = exModel.forward(input_signal=audios, input_signal_length=audio_lengths, segment_lens=batch['segment_lens'] if isfalse(args.do_not_pass_segment_lens) else None)
    ...
    print(f'Exemplar checkpoint:{args.ex_checkpoint}')
...
def main(args):
    ...
    evaluate(args, model, ami_dict[args.split], decoder, exModel = exModel)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ex_model_config', type=str, default='', help='model to use to get exemplar labels')

...
def ctc_collapse(log_probs, encoded_len, model, blankID = 39):
    labels_batch = torch.argmax(log_probs, dim = -1)
    predictions = []
    for nth, len in enumerate(encoded_len):
        prediction = []
        labels = labels_batch[nth]
        prev_label = -1
        for lth in range(len):
            # print(f'labels[lth]: {labels[lth]}')
            label = labels[lth]
            if label != prev_label and label != blankID:
                prediction.append(model.decoder.vocabulary[label.item()])
                prev_label = label
        # print(prediction)
        # strPred = " ".join((str(pred) for pred in model.decoder.vocabulary[pred]) for pred in prediction)
        # print(f'prediction: {strPred}')
...
def evaluate(args, model, corpus):  # decoder
    ...
    dataloader = tools.load_timit_eval_dataloader(
        cuts = corpus,
        max_duration = args.max_duration,
        shuffle = False
    )
    ...
    for batch_num, batch in enumerate(pbar):
        ...
        predictions.extend(ctc_collapse(log_probs=log_probs, encoded_len=encoded_len, model=model, blankID=39))
        targets_all.extend(targets)



    wer = word_error_rate(hypotheses=predictions, references=targets_all)
    print(wer)
    return wer
...
def main(args):
    ...
    #     ["/home/acp20rm/data/timit/data/TEST/DR6/MJFC0/SX133.WAV"],
    #     batch_size=1
    # )
